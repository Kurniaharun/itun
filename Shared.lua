...
function Shared.cleanupState()
    ...
    if State.tpConn then State.tpConn:Disconnect() State.tpConn = nil end
...
----------------------------------------------------------------
-- ⚡ INSTANT TP SYSTEM (Smart Tween + Instant TP)
----------------------------------------------------------------
-- Priority: Tween dulu (safe), jika dekat (<=20) pakai instant TP
-- Menunggu sampai selesai sebelum next target
-- Anti-bug dengan proper cleanup

function Shared.instantTP(targetPos)
    local char = player.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return false end
    ...
    -- Enable noclip untuk safety
    Shared.enableNoclip()
    
    -- Instant TP dengan CFrame
    hrp.CFrame = CFrame.new(targetPos)
    hrp.Velocity = Vector3.zero
    hrp.AssemblyLinearVelocity = Vector3.zero
    
    -- Wait sedikit untuk memastikan TP selesai
    task.wait(0.1)
    
    print(string.format("   ⚡ Instant TP to (%.1f, %.1f, %.1f)", targetPos.X, targetPos.Y, targetPos.Z))
    return true
end

function Shared.smartMoveTo(targetPos, stopDistance, moveSpeed, callback)
    local State = Shared.State
    local char = player.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return false end
    ...
    -- Cek distance dari LocalPlayer
    local currentPos = hrp.Position
    local distance = (targetPos - currentPos).Magnitude
    local INSTANT_TP_DISTANCE = 20
    
    -- Jika dekat, langsung instant TP
    if distance <= INSTANT_TP_DISTANCE then
        print(string.format("   📍 Target dekat (%.1f studs), menggunakan Instant TP...", distance))
        
        -- Set flag untuk mencegah multiple TP
        if State.isTping then
            print("   ⚠️ TP sedang berjalan, menunggu...")
            return false
        end
        ...
        -- Instant TP
        local success = Shared.instantTP(targetPos)
        
        -- Wait sampai benar-benar selesai
        task.wait(0.2)
        
        -- Verify position
        if hrp and hrp.Parent then
            local finalDistance = (targetPos - hrp.Position).Magnitude
            if finalDistance <= stopDistance then
                print("   ✅ Instant TP selesai!")
                State.isTping = false
                if callback then callback() end
                return true
            else
                -- Jika masih jauh, coba lagi atau fallback ke tween
                print(string.format("   ⚠️ TP belum tepat (%.1f studs), fallback ke tween...", finalDistance))
                State.isTping = false
                return Shared.smartMoveTo(targetPos, stopDistance, moveSpeed, callback)
            end
        end
    ...
    -- Jika jauh, tween dulu sampai dekat, baru instant TP
    print(string.format("   🚀 Target jauh (%.1f studs), tween dulu...", distance))
    
    if State.isTping then
        print("   ⚠️ Movement sedang berjalan, menunggu...")
        return false
    end
    ...
    local yReached = false
    local xzReached = false
    local instantTPDone = false
    
    State.moveConn = Services.RunService.Heartbeat:Connect(function()
        if instantTPDone then return end
        ...
        elseif not xzReached then
            ...
            -- Jika sudah dekat (<=20), instant TP
            if currentDistance <= INSTANT_TP_DISTANCE then
                print(string.format("   📍 Sudah dekat (%.1f studs), instant TP...", currentDistance))
                ...
                if hrp and hrp.Parent then
                    local finalDistance = (targetPos - hrp.Position).Magnitude
                    if finalDistance <= stopDistance then
                        print(string.format("   ✅ Reached target! (%.1f studs)", finalDistance))
                        instantTPDone = true
                        xzReached = true
                        State.isTping = false
                        if callback then callback() end
            ...
            elseif distanceXZ < stopDistance then
                print(string.format("   ✅ Reached target! (%.1f studs)", distanceXZ))
